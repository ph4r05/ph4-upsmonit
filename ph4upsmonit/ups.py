...
class UpsMonit:
    ...
    def argparser(self):
        ...
        parser.add_argument("-n", "--name", dest="ups_name", help="UPS name to check")
        parser.add_argument(
            "-e", "--event", dest="event", action="store_const", const=True, help="Event from the nut daemon"
        )
        parser.add_argument("-u", "--users", dest="users", nargs=argparse.ZERO_OR_MORE, help="Allowed user names")
        parser.add_argument(
            "--user-ids", dest="user_ids", nargs=argparse.ZERO_OR_MORE, type=int, help="Allowed user IDs"
        )
        parser.add_argument(
            "-t", "--chat-id", dest="chat_ids", nargs=argparse.ZERO_OR_MORE, type=int, help="Pre-Registered chat IDs"
        )
        parser.add_argument("-p", "--port", dest="server_port", type=int, default="9139", help="UDP server port")
        parser.add_argument("-f", "--fifo", dest="server_fifo", default="/tmp/ups-monitor-fifo", help="Server fifo")
        parser.add_argument("-l", "--log", dest="json_log", help="File where to store JSON logs from events")
        parser.add_argument("--nlib", dest="use_nutlib", action="store_const", const=True, help="Use nut2 network lib")
        parser.add_argument("message", nargs=argparse.ZERO_OR_MORE, help="Text message from notifier")
        return parser
        ...
        def status_internal():
            ...
            while self.is_running:
                try:
                    ...
                    time.sleep(0.1)
            logger.info("Stopping status thread")
    ...
    async def bot_cmd_log(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        async with self.notifier_telegram.handler_helper("log", update, context) as hlp:
            ...
            last_log_txt_arr = [f" - {_txt_log(x)}" % x for x in last_log]
            last_log_txt = "\n".join(last_log_txt_arr)
    ...
    def add_log(self, msg, mtype="-"):
        ...
        self.event_log_deque.append({"time": time_now, "time_fmt": time_fmt, "mtype": mtype, "msg": msg})
    ...
    def on_new_ups_state(self, ups_name, r):
        ...
        if st.last_ups_state_txt != ups_state:
            ...
            logger.info(
                f"Detected UPS change detected {ups_name}: {ups_state}, last state change: {t - old_state_change}"
            )
        ...
        if do_report or in_state_report:
            ...
            if do_report:
                self.notify_via_email_async(
                    txt_msg, f'UPS state change {ups_name} {datetime.now().strftime("%Y-%m-%d, %H:%M:%S")}'
                )
    ...
    def fetch_ups_state_upsc(self, name):
        runner = get_runner(["/usr/bin/upsc", name], shell=False)
